...
def plan_waypoints(grid, start_xy, goal_xy):
    ...
    if s == g:
        return [tuple(map(float, goal_xy))]
    ...
    # celulas -> waypoints em mm (centro da celula); ultimo = goal exato
    wps = [_center(i, j, r) for (i, j) in simp]
    wps[-1] = (float(goal_xy[0]), float(goal_xy[1]))
