...
Arquitetura híbrida (FSM) + calibração empírica $\Rightarrow$ alvo atingido sem colisões em 19,3~s, respeitando o laço de tempo real. Depuração guiada por telemetria foi decisiva.
\end{block}
\begin{block}{Trabalhos futuros}
Odometria (robô real) $\bullet$ ultrassom (antecipação) $\bullet$ VFH/DWA $\bullet$ mapa + $A^\star$ (completude) $\bullet$ análise formal de escalonabilidade (RMS).
\end{block}
\end{frame}
