...
A Seção~\ref{sec:fund} apresenta a fundamentação teórica e a revisão da literatura. A Seção~\ref{sec:metodo} descreve materiais e métodos. A Seção~\ref{sec:resultados} expõe e discute os resultados. A Seção~\ref{sec:real} documenta a evolução para o robô físico (odometria, rastreamento por ponto de controle virtual e planejamento $A^\star$ sobre mapa). A Seção~\ref{sec:conclusao} conclui e aponta trabalhos futuros.
...
Um modo de \textbf{mapeamento automático} (\texttt{--slow}) leva o robô de $A$ a $B$ em \emph{meia velocidade}, contornando os obstáculos devagar para amostrar melhor, e salva o mapa ao final. Toda a operação ocorre por uma \textbf{interface web} (Flask) que conecta ao robô por USB (console serial) ou Wi-Fi/SSH, faz \emph{deploy} e recompilação do \texttt{main.c} no próprio robô, expõe telemetria, controle manual, mapeamento e os fluxos acima. Do ponto de vista de \textbf{tempo real}, o laço de controle embarcado permanece uma tarefa periódica (período nominal de $40$~ms), preservando o modelo discutido na Seção~\ref{sec:fund}.

% =====================================================================
\section{Conclusão e Trabalhos Futuros}\label{sec:conclusao}
% =====================================================================
Apresentou-se um controlador de navegação ponto-a-ponto com desvio de obstáculos para o Khepera~IV em ambiente Webots, fundamentado na literatura clássica de robótica móvel e validado com dados reais de execução. A arquitetura híbrida arbitrada por máquina de estados finitos, aliada à calibração empírica do sensoriamento, conduziu o robô ao alvo de forma livre de colisões em $19{,}3$~s, reduzindo a distância de $2{,}83$~m a menos de $0{,}07$~m. O processo de depuração baseado em telemetria mostrou-se tão instrutivo quanto o resultado, evidenciando os fenômenos de mínimo local e de descalibração sensorial. A Seção~\ref{sec:real} documentou ainda a evolução para o \textbf{robô físico}, na qual vários dos trabalhos futuros abaixo já foram concretizados: odometria, rastreamento de linha por ponto de controle virtual e uma camada deliberativa de mapeamento com planejamento $A^\star$.
